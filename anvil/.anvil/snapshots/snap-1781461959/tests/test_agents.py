...
class TestBaseAgent:
    ...
    def test_available_tools_whitelist_over_blacklist(self):
        agent = BaseAgent(tools_whitelist=["bash"], tools_blacklist=["bash"])
        result = agent.available_tools(ALL_TOOLS)
        assert "bash" in result
...
class TestAgentManager:
    ...
    def test_list_includes_hidden(self):
        mgr = AgentManager()
        all_agents = mgr.list(hidden=True)
    ...
    def test_current_after_switch(self):
        mgr = AgentManager()
        mgr.switch("build")
        assert mgr.current.name == "build"
        mgr.switch("plan")
        assert mgr.current.name == "plan"
...
You are a helpful markdown agent."""
        agent = mgr.from_markdown(markdown, name="md_agent")
        assert agent.name == "md_agent"
        assert "markdown" in agent.description.lower()
    ...
    def test_load_from_directory(self, tmp_path):
        ...
        rules_dir.mkdir()
        (rules_dir / "searcher.md").write_text("# Agent: searcher\ndescription: Search agent\nmodel: local\n---\nSearch well.")
        agents = mgr.load_from_directory(rules_dir)
        assert len(agents) >= 1
        assert mgr.get("searcher") is not None

    def test_load_from_nonexistent_directory(self):
        mgr = AgentManager()
        agents = mgr.load_from_directory(path=__import__("pathlib").Path("/nonexistent"))
        assert agents == []
...
class TestMentionParsing:
    def test_parse_single_mention(self):
        ...
        mentions = AgentManager.parse_mentions(text)
        assert len(mentions) >= 1
        assert mentions[0][0] == "explore"

    def test_parse_multiple_mentions(self):
        text = "@explore find auth @plan analyze this"
        mentions = AgentManager.parse_mentions(text)
        assert len(mentions) >= 1
