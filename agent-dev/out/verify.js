...
class VerifyPhase {
    ...
    async run_verification(code, expectedBehavior, filePath) {
        const syntaxErrors = [];
        ...
        const allPassed = !hasSyntaxErrors &&
        ...
        if (ext === '.ts' || ext === '.tsx') {
            try {
                ...
                errors.push({
                    ...
                    line: 0,
                    column: 0,
                    message: `Syntax check failed: ${err.message}`,
                    severity: 'warning',
                });
            }
        }
    ...
    async check_syntax_directory(dirPath) {
        const errors = [];
        const workspacePath = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
        const tscPath = path.join(workspacePath || '', 'node_modules', '.bin', 'tsc');
        try {
            ...
            errors.push({
                ...
                line: 0,
                column: 0,
                message: `Directory syntax check failed: ${err.message}`,
                severity: 'warning',
            });
        }
        return errors;
    }
    async check_tests(testCommand) {
        const workspacePath = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
        try {
    ...
    async check_lint(fileOrCommand) {
        const workspacePath = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
        try {
    ...
    exec_command(command, cwd, timeout = 30000) {
